...
import FlterSeasonActivity from "./FlterSeasonActivity";
import {
    HomeContainer,
    HomeButton
} from "../styles/styles";
...
export default function Home() {
    ...
    const [filtroSeason, setFiltroSeason] = useState("");
    const [countriesSeasonActvity, setCountriesSeasonActivity] = useState([]);
    ...
    const finalPageFilterSeason = Math.floor(countriesSeasonActvity / 10);
    ...
    const filterSeasonActivity = () => {
        if (countriesDesc.length) setCountriesDesc([]);
        if (countriesPobl.length) setCountriesPobl([]);
        setCountriesSeasonActivity(countries.filter(c => {
            return c.Activities.season === filterSeasonActivity
        }));
    };
    ...
    return (
        <HomeContainer>
            ...
            {countriesSeasonActvity.length && <Countries 
            countries={countriesSeasonActvity}
            finalPage={finalPageFilterSeason}
            />}
